...
#             total_loss = 0.0
def perform_training(training_loader, vision_model, model_optimizer, loss_function, device, num_epochs=20, mix_sampling_method=1):
    vision_model.train()
    for epoch in range(num_epochs):
        ...
        for batch_index, (x_input, y_label) in enumerate(training_loader, 0):
            ...
            # MixUp augmentation
            mixup = MixUp(mix_sampling_method)  
            ...
            print(f'Epoch: {epoch + 1}, Batch: {batch_index + 1}, Batch Loss: {batch_loss.item():.3f}')

        print(f'Epoch: {epoch + 1}, Training Loss: {total_loss / len(training_loader):.3f}')
        
    # After training loop
    torch.save(vision_model.state_dict(), f'model_{mix_sampling_method}.pth')
...
if __name__ == '__main__':
    ...
    for sampling_method in [1, 2]:
        ...
        # Training
        perform_training(trainloader, model, optimizer, criterion, device, num_epochs,sampling_method)
